...
namespace AplicatieConcediu.Pagini_Actiuni
{
    public partial class FormareEchipaAngajatPromovat : Form
    {
        ...
        private void FormareEchipaAngajatPromovat_Load(object sender, EventArgs e)
        {
            ...
            SqlDataReader reader = Globals.executeQuery("Select Nume, Prenume, Email,DataAngajarii, DataNasterii, CNP, IdEchipa, ManagerId from Angajat where ManagerId is not null and Email is not '"+Globals.EmailManager+"'", out conn);
            while (reader.Read())
            {
